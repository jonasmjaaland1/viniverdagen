...
export default function LoginSide() {
  ...
  const [magicLinkLogger, setMagicLinkLogger] = useState(false);
  const router = useRouter();
  const supabase = createClient();

  // Håndter magic link med #access_token i URL
  useEffect(() => {
    async function handleMagicLink() {
      ...
      if (!accessToken || !refreshToken) {
        setMagicLinkLogger(false);
        setFeil("Magisk lenke mangler nødvendige felter.");
        return;
      }
      ...
      if (sesErr) {
        setMagicLinkLogger(false);
        setFeil("Kunne ikke logge inn: " + sesErr.message);
        return;
      }
      ...
      if (user) {
        ...
        if (!medlem?.godkjent) {
          ...
          setMagicLinkLogger(false);
          setFeil("Kontoen din er ikke godkjent ennå.");
          return;
        }
      }

      // Fjern hash og redirect
  ...
  async function loggInn(e: React.FormEvent) {
    ...
    if (user) {
      ...
      if (!medlem?.godkjent) {
        await supabase.auth.signOut();
        setFeil(
          "Kontoen din er ikke godkjent ennå. Du blir kontaktet når den er aktivert.",
        );
        setLaster(false);
        return;
      }
  ...
  // Vis spinner mens magic link logger inn
  if (magicLinkLogger) {
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="text-ink-700/70 italic">Logger deg inn...</p>
        </div>
      </div>
    );
  }
