...
export default function ForslagKort({
  ...
  async function svar(
    ...
    const { error } = await supabase.from("dato_svar").upsert(
      {
  ...
  async function bekreft(alternativ: Alternativ) {
    if (!erAdmin) return;
    if (!confirm(`Bekrefte ${formatDato(alternativ.dato)} som klubbkveld?`))
      return;
    setLaster("bekreft");
    ...
    const tittelTekst =
      forslag.tittel || `Klubbkveld ${formatDato(alternativ.dato)}`;

    // KORRIGERT: bruker 'tittel' og 'kommentar' (de faktiske kolonnenavnene)
    const { data: kveld, error: e1 } = await supabase
      .from("klubbkvelder")
      .insert({
        tittel: tittelTekst,
        dato: alternativ.dato,
        kommentar: forslag.beskrivelse || null,
    ...
    if (e1 || !kveld) {
      alert(
        "Kunne ikke opprette klubbkveld: " + (e1?.message || "Ukjent feil"),
      );
      setLaster(null);
      return;
    }
    ...
    if (e2) {
      alert(
        "Klubbkveld opprettet, men kunne ikke oppdatere forslag-status: " +
          e2.message,
      );
    }
  ...
  async function fjernDato(alternativId: string, dato: string) {
    ...
    if (
      !confirm(
        `Fjerne ${formatDato(dato)} fra forslaget? Alle svar på denne datoen forsvinner.`,
      )
    ) {
      return;
    }
    ...
    <article className="kort p-6 md:p-8 space-y-5">
          ...
          <div className="min-w-0">
            ...
            {forslag.ansvarlig_navn && (
              <p className="text-sm font-sans text-wine-700 mt-2">
                Ansvarlig:{" "}
                <span className="font-medium">{forslag.ansvarlig_navn}</span>
              </p>
            )}
          </div>
      ...
      <div className="space-y-2">
        {sortertAlternativer.map((alt) => {
          ...
          const erBeste =
            beste && beste.id === alt.id && forslag.status === "apen";
          const erBekreftet =
            forslag.bekreftet_klubbkveld_id &&
            forslag.status === "bekreftet" &&
            alt.dato === forslag.bekreftet_dato;
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  <p className="font-display text-lg text-wine-900">
                    {formatDato(alt.dato)}
                    {erBekreftet && (
                      <span className="ml-2 text-sm text-wine-700">
                        ✓ valgt
                      </span>
                    )}
                  </p>
                  <div className="flex gap-3 text-xs font-sans text-ink-700/70 mt-1 flex-wrap">
                    ...
                    {alt.antall_kanskje > 0 && (
                      <span className="text-amber-700">
                        ~ {alt.antall_kanskje} kanskje
                      </span>
                    )}
                    {alt.antall_kan_ikke > 0 && (
                      <span className="text-ink-700/50">
                        ✕ {alt.antall_kan_ikke} kan ikke
                      </span>
                    )}
                  </div>
                </div>
