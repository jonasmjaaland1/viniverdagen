    ...
    <section className="kort p-6 md:p-8 mb-8 border-l-4 border-wine-700 fade-up">
      ...
      {forslag.map((f) => {
        ...
        const beste = sortertAlt.reduce<Alternativ | null>(
          (best, alt) =>
            !best || alt.antall_kan > best.antall_kan ? alt : best,
          null,
        );
        ...
        return (
          <div key={f.id} className="space-y-3">
            ...
            {f.beskrivelse && (
              <p className="text-sm font-sans text-ink-700/80 italic">
                {f.beskrivelse}
              </p>
            )}
            {f.ansvarlig_navn && (
              <p className="text-xs font-sans text-wine-700">
                Ansvarlig:{" "}
                <span className="font-medium">{f.ansvarlig_navn}</span>
              </p>
            )}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      ...
                      <div className="flex gap-1">
                        ...
                        <button
                          onClick={() =>
                            svar(alt.id, "kan_ikke", alt.mitt_svar)
                          }
                          disabled={laster !== null}
                          className={`px-2.5 py-1 text-[11px] uppercase tracking-wider font-sans rounded transition ${
